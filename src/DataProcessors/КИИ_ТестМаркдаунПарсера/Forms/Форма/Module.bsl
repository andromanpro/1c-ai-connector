...
Процедура ОбновитьHTML()
	...
	Исключение
		...
		ОписаниеОшибкиHTML = КИИ_МаркдаунПарсерКлиентСервер.СобратьHTMLБлокаОшибки(
			НСтр("ru = 'ОШИБКА:'"),
			ТекстОшибки);
...
Функция ПолучитьПримерБазовый()
	...
	Возврат НСтр("ru = '# Заголовок 1 уровня
		...
		|
		|Текст с ~~зачеркиванием~~ и `inline кодом`.
		|
...
Функция ПолучитьПримерВсе()
	...
	Возврат НСтр("ru = '# Полный тест Markdown
		...
		|
		|**Жирный**, *курсив*, ***жирный курсив***, ~~зачеркнутый~~, `код`.
		|
		|## Списки
		|
		|1. Первый
